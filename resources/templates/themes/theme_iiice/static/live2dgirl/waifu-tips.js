...
$('.waifu-tool .fui-cross').click(function (){
    ...
    window.setTimeout(function() {$('#live2d').hide();}, 1300);
    isHiddenWaifu = true;
});
...
function initModel(waifuPath){
    if(!isHiddenWaifu) {
        showMessage('其实，我就是你要的萌妹啦 ^_^', 1300, true);
        return;
    }
    ...
    if (modelId == null) {
        ...
        $("#live2d").show();
        sessionStorage.setItem('waifu-dsiplay', 'normal');
        showMessage('Hello，我们又见面啦 ^_^', 1300, true);
    }
    loadModel(modelId, modelTexturesId);
    loadWaifuTips(waifuPath);
    isHiddenWaifu = false;
}

var isLoadTips = false;
var isHiddenWaifu = true;
function loadWaifuTips(waifuPath){
  if(isLoadTips){
      return;
  }
  $.ajax({
        ...
        success: function (result){
            ...
            $.each(result.mouseover, function (index, tips){
                $(document).on("mouseover", tips.selector, function (){
                    if(tips.selector == ".waifu #live2d" && isHiddenWaifu) return;
                    var text = tips.text;
